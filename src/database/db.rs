...
impl Database {
	...
	pub async fn execute(
		&self, query: &str, params: &[&(dyn tokio_postgres::types::ToSql + Sync)],
	) -> Result<u64, Error> {
	...
	pub async fn insert(
		&self, table: &str, columns: &[&str], values: &[&(dyn tokio_postgres::types::ToSql + Sync)],
	) -> Result<u64, Error> {
	...
	pub async fn select(
		&self, table: &str, columns: &[&str], where_clause: Option<&str>,
		where_values: &[&(dyn tokio_postgres::types::ToSql + Sync)],
	) -> Result<Vec<Row>, Error> {
	...
	pub async fn select_one(
		&self, table: &str, columns: &[&str], where_clause: &str,
		where_values: &[&(dyn tokio_postgres::types::ToSql + Sync)],
	) -> Result<Row, Error> {
		let query = format!(
			"SELECT {} FROM {} WHERE {}",
